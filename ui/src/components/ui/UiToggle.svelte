<script lang="ts">
	...
	const toggleVariants = tv({
		...
		variants: {
			variant: {
				primary: 'bg-primary text-white hover:bg-primary/90 rounded-[0.5vh] data-[pressed=true]:bg-primary/80 data-[pressed=true]:text-accent',
				secondary: 'bg-secondary text-white hover:bg-secondary/90 rounded-[0.5vh] border border-primary data-[pressed=true]:bg-secondary/80 data-[pressed=true]:text-accent',
				icon: 'bg-primary text-white hover:bg-primary/90 rounded-[0.5vh] data-[pressed=true]:bg-primary/80 data-[pressed=true]:text-accent',
				row: 'w-full bg-tertiary text-white hover:bg-tertiary/90 rounded-[0.5vh] justify-start px-[1.5vh] data-[pressed=true]:bg-tertiary/80 data-[pressed=true]:text-accent'
			},
			size: {
	...
	interface Props extends Omit<ToggleVariants, 'hasTooltip'> {
		pressed?: boolean
		children?: import('svelte').Snippet
		onclick?: (pressed: boolean, event: MouseEvent) => void
		tooltipText?: string
	}
	...
	function handleClick(event: MouseEvent) {
		if (disabled) return
		pressed = !pressed
		onclick(pressed, event)
	}
</script>

<button
	disabled={disabled}
	class={toggleVariants({ variant, size, disabled, hasTooltip })}
